...
/// <summary>Maps Keycloak realm and client roles to ASP.NET Core role claims.</summary>
public sealed class KeycloakRoleClaimsTransformation(IConfiguration configuration) : IClaimsTransformation
{
    public Task<ClaimsPrincipal> TransformAsync(ClaimsPrincipal principal)
    {
        ...
        var clientId = configuration["Jwt:ClientId"] ?? "pet-service";
        ...
        try
        {
            ...
            if (clientId is not null
                && (access.ValueKind != JsonValueKind.Object
                    || !access.TryGetProperty(clientId, out access)))
            {
                return [];
            }
